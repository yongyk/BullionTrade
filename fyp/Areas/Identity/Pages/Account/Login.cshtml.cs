...
namespace fyp.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (string.IsNullOrEmpty(recaptchaResponse) || !ReCaptchaPassed(recaptchaResponse))
            {
        ...
        private bool ReCaptchaPassed(string gRecaptchaResponse)
        {
            using (var client = new HttpClient())
            {
                ...
                var response = client.PostAsync($"https://www.google.com/recaptcha/api/siteverify?secret={secret}&response={gRecaptchaResponse}", new StringContent("")).Result;
                if (response.IsSuccessStatusCode)
                {
                    var JSONres = response.Content.ReadAsStringAsync().Result;
                    dynamic JSONdata = JObject.Parse(JSONres);
                    return JSONdata.success == "true"; // Correctly check the success status
                }
            }
            return false;
        }
